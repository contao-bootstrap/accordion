...
final class AccordionWrapperMigration extends AbstractMigration
{
    ...
    public function shouldRun(): bool
    {
        ...
        return $queryBuilder
                ...
                ->setParameter('type', 'bs_accordion_start')
                ->executeQuery()
                ->fetchOne() > 0;
    }
	...
	SELECT
		...
		(
			...
			WHERE accordion_stop.pid     = accordion_start.pid
			  ...
			  AND accordion_stop.type    = 'bs_accordion_end'
	...
	WHERE accordion_start.type = 'bs_accordion_start'
...
SQL;
        ...
        $groupContainers = array_reduce($accordionGroups, static function (array $carry, array $row) {
        ...
        foreach ($groupContainers as $gridContainer) {
            $this->connection->update(
                'tl_content',
                ['type' => 'bs_accordion_wrapper', 'ptable' => 'tl_content'],
        ...
        return $this->createResult(
            true,
            'Migrated ' . count($groupContainers) . ' accordion containers and ' . $elementCount . ' elements.',
        );
    }
}
